# This file was auto-generated by Fern from our API Definition.

import datetime as dt
import typing

import pydantic
import typing_extensions
from ...core.pydantic_utilities import IS_PYDANTIC_V2, UniversalBaseModel
from ...core.serialization import FieldMetadata
from .get_orders_id_lines_response_data_item_order_line_attribute_item import (
    GetOrdersIdLinesResponseDataItemOrderLineAttributeItem,
)


class GetOrdersIdLinesResponseDataItem(UniversalBaseModel):
    id: str
    product_id: typing_extensions.Annotated[str, FieldMetadata(alias="productId")]
    name: str
    description: typing.Optional[str] = None
    start_date: typing_extensions.Annotated[dt.datetime, FieldMetadata(alias="startDate")]
    end_date: typing_extensions.Annotated[typing.Optional[dt.datetime], FieldMetadata(alias="endDate")] = None
    order_line_attribute: typing_extensions.Annotated[
        typing.List[GetOrdersIdLinesResponseDataItemOrderLineAttributeItem], FieldMetadata(alias="OrderLineAttribute")
    ]

    if IS_PYDANTIC_V2:
        model_config: typing.ClassVar[pydantic.ConfigDict] = pydantic.ConfigDict(extra="allow", frozen=True)  # type: ignore # Pydantic v2
    else:

        class Config:
            frozen = True
            smart_union = True
            extra = pydantic.Extra.allow
