...
All other API client code (customers, products, contacts, orders, invoices, signals) is auto-generated and should not be modified directly.
...
The `Paid` and `AsyncPaid` client classes are auto-generated API clients with resource clients:
...
- `signals` - Signal creation for usage tracking

Note: Tracing functionality is accessed via the `paid.tracing` module, not the client:

- `paid.tracing.initialize_tracing()` - Initialize OTEL tracing
- `paid.tracing.paid_tracing` - Decorator/context manager for tracing
- `paid.tracing.signal()` - Emit signals within a tracing context

### Auto-Generated Code (Do Not Modify)

- **customers/**, **products/**, **contacts/**, **orders/**, **invoices/**, **signals/** - API resource clients
...
3. **Distributed Tracing**: Supports linking traces across processes/machines:
   ...
   - Pass token to `@paid_tracing(tracing_token=token)` or context manager `with paid_tracing(tracing_token=token)`
   - Creates `NonRecordingSpan` with custom `SpanContext` to link traces

4. **Flexible Tracing API**: Two ways to add tracing via `paid.tracing.paid_tracing`:

   - Decorator: `@paid_tracing("customer_id", external_product_id="product_id")`
   - Context manager: `with paid_tracing("customer_id", external_product_id="product_id"): ...`
...
- `src/paid/customers/`, `products/`, `contacts/`, `orders/`, `invoices/`, `signals/` - API resource clients
