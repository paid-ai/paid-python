"""Pydantic-AI + Google GenAI + Paid tracing tests. Verifies span attributes match agent results.

Record cassettes: GEMINI_API_KEY=... poetry run pytest tests/tracing/test_pydantic_ai_gemini.py --record-mode=once -rP
"""
...
def _assert_span_matches(span, result):
    ...
    usage = result.usage()
    assert attrs.get(ATTR_GEN_SYSTEM) == "google-gla"
...
class TestGeminiStructuredOutput:
    ...
    def test_structured_output_pydantic_model(self, tracing_setup, tracing_provider):
        exporter = tracing_setup
        agent = Agent(GEMINI_PYDANTIC_MODEL, output_type=CityInfo, instrument=_instr(tracing_provider))
    ...
    def test_structured_output_list_of_strings(self, tracing_setup, tracing_provider):
        exporter = tracing_setup
        agent = Agent(GEMINI_PYDANTIC_MODEL, output_type=list[str], instrument=_instr(tracing_provider))
...
class TestGeminiSystemPrompts:
    ...
    def test_dynamic_system_prompt(self, tracing_setup, tracing_provider):
        ...
        exporter = tracing_setup
        agent = Agent(GEMINI_PYDANTIC_MODEL, deps_type=UserDeps, instrument=_instr(tracing_provider))
...
class TestGeminiPydanticSpanProcessor:
    ...
    def test_track_usage_false(self, tracing_setup, tracing_provider):
        exporter = tracing_setup
        agent = Agent(GEMINI_PYDANTIC_MODEL, instrument=_instr_no_usage(tracing_provider))
        agent.run_sync("Say hello.")
        attrs = dict(_gen_ai_spans(exporter)[0].attributes)
...
class TestGeminiCombinedInstrumentation:
    ...
    @pytest.mark.vcr()
    async def test_pydantic_stream_with_google_genai_autoinstrumentation(self, tracing_setup, tracing_provider):
        exporter = tracing_setup
        paid_autoinstrument(libraries=["google-genai"])
        agent = Agent(GEMINI_PYDANTIC_MODEL, instrument=_instr(tracing_provider))
